package com.github.liguanyu.claudecodelauncher.http

import com.github.liguanyu.claudecodelauncher.files.FileOpenService
import com.github.liguanyu.claudecodelauncher.notifications.NotificationService
...
class HttpTriggerService : Disposable {
            ...
            if (requestMethod == HTTP_METHOD_POST) {
                ...
                if (requestBody.isNotEmpty()) {
                    try {
                        ...
                        val hookEventName = json["hook_event_name"]?.jsonPrimitive?.contentOrNull
                        val legacyType = json["type"]?.jsonPrimitive?.contentOrNull
                        if (hookEventName == "Stop" || hookEventName == "StopFailure" || legacyType == "agent-turn-complete") {
                            val notificationMessage = buildNotificationMessage(json, hookEventName)
                            ApplicationManager.getApplication().executeOnPooledThread {
                                processRefreshRequest(notificationMessage)
                            }
                        } else {
                            logger.warn("Ignoring notification with unsupported hook event: $hookEventName")
                        }
                    } catch (e: Exception) {
    ...
    private fun buildNotificationMessage(json: JsonObject, hookEventName: String?): String {
        val baseMessage = json["last_assistant_message"]?.jsonPrimitive?.contentOrNull
            ?: json["last-assistant-message"]?.jsonPrimitive?.contentOrNull
        ...
        val details = json["error_details"]?.jsonPrimitive?.contentOrNull
        return listOf(baseMessage, error, details)
    ...
    private fun enqueueProjectProcessing(
        project: Project,
        settings: ClaudeCodeLauncherSettings,
    ...
    private fun sendResponse(exchange: HttpExchange, statusCode: Int, response: String) {
        exchange.responseHeaders.set("Content-Type", CONTENT_TYPE_JSON)
